 ...
 *
 * The Template for displaying 2 column portfolio
 *
 * @package WordPress
 * @subpackage Quezal
 * @since Quezal 1.0
...
//            echo "<pre>"; print_r($terms);
						...
						echo '<li class="all-link"><a class="active" href="http://al-burraq.com/portfolio/" data-filter="*">' . $all . '</a></li>';
							if ($count > 0) {
                ...
                $my_terms[] = $terms[2];
    							foreach ($my_terms as $term) {
        							...
        							echo '<li><a href="#" title="" onclick="filter_me(`'.$termslug.'`,0)" data-filter=".' . $termname . '">' . $term->name . '</a></li>';
    							}
							}
						echo "</ul>"; ?>
        <?php } ?>
        ...
        <div class="col-md-12">
            <img class="bloader" src="<?php echo get_template_directory_uri() . '/img/bx_loader.gif'; ?>" />
        </div>
      ...
      <?php 
			...
			$loop = new WP_Query( array(
				...
				'posts_per_page' => ( $tcsn_option['tcsn_portfolio_items_per_page'] ?  /*$tcsn_option['tcsn_portfolio_items_per_page'] */ 10: 10),
				'paged'          => $paged,
				'order'          => ( $tcsn_option['tcsn_portfolio_arrange'] ?  /*$tcsn_option['tcsn_portfolio_arrange']*/ 'ASC' : 'ASC'),   // DESC or ASC 
				'orderby'        => ( $tcsn_option['tcsn_portfolio_sort'] ?  $tcsn_option['tcsn_portfolio_sort'] : 'date'),   // date, rand or title
			
				) );

     // echo "<pre>"; print_r($loop->have_posts());
			?>
      <div id="items" class="filter-content">
        ...
        <?php
					...
					$tax = ''; ?>
        <?php endif; ?>
        <div class="col-md-6 col-sm-6 col-xs-12 portfolio-item isotope-item <?php echo strtolower($tax); ?> all">
          <?php 
