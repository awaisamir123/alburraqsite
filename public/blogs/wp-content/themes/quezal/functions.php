...
// Layout
require_once( get_template_directory() .'/includes/templates/layout.php' );
...
if(!function_exists('tcsn_woocommerce_scripts_styles')) {
	function tcsn_woocommerce_scripts_styles() {
		if(is_woocommerce_activated()) {
			wp_enqueue_style( 'woocommerce-style',  get_template_directory_uri() . '/css/woocommerce.css' );
		}
	}
	add_action('wp_enqueue_scripts', 'tcsn_woocommerce_scripts_styles');
}
...
function woocommerce_header_add_to_cart_fragment( $fragments ) {
	...
	<div class="cart-contents">
         <span class="shopping-cart-items"><?php echo sprintf(_n('<span class="cart-item">Item: </span> %d', '<span class="cart-item">Items: </span>%d ', $woocommerce->cart->cart_contents_count, 'woothemes'), $woocommerce->cart->cart_contents_count);?></span>
...
function tcsn_team_sc( $atts, $content = null ) {
	...
	if( have_posts() ) :
		...
		while ( have_posts() ) : the_post();
			...
			$output .= '<div class="member-image"><a href="' . $permalink . '" rel="bookmark">' . get_the_post_thumbnail($post->ID, 'full') . '</a></div>';
